layui.use(['form', 'layer'], function () {
    ...
    $('#submitBtn').on('click', function() {
        ...
        // 使用MD5加密密码
        let encryptedOldPwd = md5(oldPassword);
        let encryptedNewPwd = md5(newPassword);
            ...
            success: function(res) {
                ...
                if (res.success) {
                    layer.alert('密码修改成功！请使用新密码重新登录', {
                        ...
                        $('#strengthText').text('密码强度：请输入密码').css('color', '#888');
                    });
                } else {
    ...
    function updateStrengthIndicator(strength) {
        ...
        switch(strength) {
            case 'weak':
                $bar.addClass('weak');
                $text.text('密码强度：弱').css('color', '#ff6b6b');
                break;
            case 'medium':
                $bar.addClass('medium');
                $text.text('密码强度：中').css('color', '#feca57');
                break;
            case 'strong':
                $bar.addClass('strong');
                $text.text('密码强度：强').css('color', '#1dd1a1');
                break;
            default:
                $text.text('密码强度：请输入密码').css('color', '#888');
        }
    }
});
